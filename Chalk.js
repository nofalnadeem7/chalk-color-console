import chalk from "chalk";

console.log(chalk.blue("This text is blue"));
console.log(chalk.red("This text is red"));
console.log(chalk.yellow("This text is yellow"));
console.log(chalk.green("This text is green"));

console.log("------------------------");

console.log(chalk.bold("This text is bold"));
console.log(chalk.underline("This text is underline"));
console.log(chalk.bgCyanBright("This text has a CyanBright background"));
console.log(chalk.blueBright("This text is bluebright"));
console.log(chalk.blackBright("This text is blackbright"));
console.log(chalk.grey("This text is grey"));
console.log(chalk.black("This text is black"));
console.log(chalk.bgWhite("This text has white background"));
